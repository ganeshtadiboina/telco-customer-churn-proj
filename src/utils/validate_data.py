...
def validate_telco_data(df) -> Tuple[bool, List[str]]:
    ...
    It validates data integrity, business logic constarints, and statistical properties that the Ml model expects.
    ...
    # Financial features (key churn predictiors)
    ...
    print("Validating business logic constarints...")
    ...
    # Contract types must be valid (business constariants)
    ge_df.expect_column_values_to_be_in_set(
        "Contract",
        ["Month-to-month", "One year", "Two year"]
    )

    # Internet service types (business constarint)
    ge_df.expect_column_values_to_be_in_set(
        "InternetService",
        ["DSL", "Fiber optic", "No"]
    )
    ...
    # Total charges should generally be >= Monthly charges (expect for very new customers)
    ...
    # === PROCESS RESULTS ===
    # Extract failed expectations for detailed error reposting
    failed_expectations = []
    ...
    if results["success"]:
        print(f"Data validate PASSED: {passed_checks}/{total_checks} checks successful")
    else:
